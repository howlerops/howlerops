  ...
  persist(
    (set, get) => ({
      ...
      signUp: async (username: string, email: string, password: string) => {
        return dedupedRequest(`signUp-${email}`, async () => {
      ...
      signIn: async (username: string, password: string) => {
        return dedupedRequest(`signIn-${username}`, async () => {
          set({ isLoading: true, error: null })
      ...
      signInWithOAuth: async (provider: 'google' | 'github') => {
        return dedupedRequest(`signInWithOAuth-${provider}`, async () => {
          set({ isLoading: true, error: null })
      ...
      signInWithBiometric: async () => {
        return dedupedRequest('signInWithBiometric', async () => {
          set({ isLoading: true, error: null })
      ...
      checkStoredAuth: async () => {
        return dedupedRequest('checkStoredAuth', async () => {
      ...
      refreshToken: async () => {
        return dedupedRequest('refreshToken', async () => {
          const { tokens } = get()
