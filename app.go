...
func NewApp() *App {
	...
	webauthnManager, err := auth.NewWebAuthnManager(credentialStore, sessionStore)
	if err != nil {
...
func (a *App) GetOAuthURL(provider string) (map[string]string, error) {
	a.logger.WithField("provider", provider).Info("Generating OAuth URL")
	...
	if err != nil {
		a.logger.WithError(err).WithField("provider", provider).Error("Failed to generate OAuth URL")
		return nil, err
	}

	a.logger.WithField("provider", provider).Info("OAuth URL generated successfully")
	return result, nil
}

// Logout removes the stored token for the specified provider
func (a *App) Logout(provider string) error {
	a.logger.WithField("provider", provider).Info("Logging out")
	...
	if err := a.secureStorage.DeleteToken(provider); err != nil {
		a.logger.WithError(err).WithField("provider", provider).Error("Failed to logout")
		return err
	}

	a.logger.WithField("provider", provider).Info("Logged out successfully")
	return nil
}

// CheckStoredToken checks if a valid token exists for the specified provider
func (a *App) CheckStoredToken(provider string) (bool, error) {
	a.logger.WithField("provider", provider).Debug("Checking for stored token")
	...
	if err != nil {
		a.logger.WithError(err).WithField("provider", provider).Error("Failed to check stored token")
		return false, err
	}
...
func (a *App) GetStoredUserInfo(provider string) (map[string]interface{}, error) {
	a.logger.WithField("provider", provider).Debug("Retrieving stored user info")
	...
	if err != nil {
		a.logger.WithError(err).WithField("provider", provider).Error("Failed to retrieve stored token")
		return nil, err
	}
	...
	a.logger.WithField("provider", provider).Debug("Retrieved stored user info")
	return userInfo, nil
}

// OnUrlOpen handles custom protocol URL callbacks (e.g., OAuth redirects)
func (a *App) OnUrlOpen(url string) {
	a.logger.WithField("url", url).Info("Handling URL open callback")

	// Check if this is an OAuth callback
	if !strings.Contains(url, "howlerops://auth/callback") {
		a.logger.WithField("url", url).Debug("Not an OAuth callback URL, ignoring")
		return
	}
...
func (a *App) handleOAuthCallback(code, state string) {
	a.logger.Info("Processing OAuth callback")
	...
	if a.githubOAuth != nil {
		...
		if err == nil {
			...
			a.logger.WithError(err).Error("GitHub OAuth exchange failed")
		}
	}
	...
	if user == nil && a.googleOAuth != nil {
		...
		if err == nil {
			...
			// Real error (not just wrong provider)
			a.logger.WithError(err).Error("Google OAuth exchange failed")
		}
	}
...
func (a *App) StartWebAuthnRegistration(userID, userName string) (string, error) {
	a.logger.WithFields(logrus.Fields{
		"userID":   userID,
		"userName": userName,
